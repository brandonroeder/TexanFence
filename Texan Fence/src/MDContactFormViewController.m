...
- (void)buildView:(NSDictionary *)viewDictionary {
    ...
    if ([self isString:message]) {
        [self addLabel:message alignment:NSTextAlignmentCenter];
    }
        ...
        for (NSInteger i = 0; i < [inputItems count]; i++) {
            ...
            if ([self isDictionary:itemDictionary]) {
                ...
                if ([self isString:displayName]) {
                    [self addLabel:displayName alignment:NSTextAlignmentLeft];
                }
...
- (void)addLabel:(NSString *)text alignment:(NSTextAlignment)textAlignment {
